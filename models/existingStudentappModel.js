...
const createExistingStudentDetails = async (req, res) => {
  try {
    ...
    const passportFile = req.file ? req.file.path : null; // Assuming passport is uploaded as single file
    const signatureFile = req.files?.signature
      ? req.files.signature[0].path
      : null; // Assuming multiple files are handled
    ...
    const student_id = studentResult.rows[0].student_id;  // Fixed typo


    // Step 4: Store passport and signature file paths in the file_uploads table
    const fileUploadQuery = `
            INSERT INTO file_uploads (student_id, file_name, file_path)
